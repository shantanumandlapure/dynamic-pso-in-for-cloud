...
// global variable to track the number of iterations without improvement
int noImprovementCount = 0;
int MAX_NO_IMPROVEMENT = 125; // Adjustment can be accrding to requirement 
...
void dynamicResourceAllocation() {
    ...
    for (int i = 0; i < NUM_SERVERS; ++i) {
        ...
        if (servers[i].cpuUtilization > 0.75) {
...
int main() {
    ...
    for (int iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
        int improved = 0; // Track if any particle improved

        for (int i = 0; i < NUM_PARTICLES; ++i) {
            updateParticle(&particles[i]);

            // Check if the particle's personal best improved
            if (particles[i].fitness > evaluateFitness(&particles[i])) {
                improved = 1;
            }
        }

        // If no particle improved, increment the no improvement count
        if (!improved) {
            noImprovementCount++;
        } else {
